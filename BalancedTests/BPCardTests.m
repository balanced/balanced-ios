...
- (void)testCreateCard {
    ...
    BPCard *card = [[BPCard alloc] initWithNumber:cardNumber expirationMonth:8 expirationYear:2025];
...
- (void)testCreateCardWithOptionalFields {
    ...
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                        ...
                                        nil];
    BPCard *card = [[BPCard alloc] initWithNumber:cardNumber
                                  expirationMonth:8
                                   expirationYear:2025
                                   optionalFields:optionalFields];
    STAssertTrue([card.number isEqualToString:cardNumber], @"Card number should equal %@", cardNumber);
    STAssertTrue([[[card optionalFields] valueForKey:@"name"] isEqualToString:name], @"Name should be %@", name);
    STAssertTrue([[[card optionalFields] valueForKey:@"security_code"] isEqualToString:securityCode], @"Security code should be %@", securityCode);
    STAssertTrue([[[card optionalFields] valueForKey:@"phone_number"] isEqualToString:phoneNumber], @"Phone number should be %@", phoneNumber);
}

- (void)testCreateCardWithOptionalFieldsWithOptionalKeyConstants {
    NSString *name = @"Johann Bernoulli";
    NSString *phoneNumber = @"111-222-3333";
    NSString *cardNumber = @"4242424242424242";
    NSString *securityCode = @"123";
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    name, BPCardOptionalParamNameKey,
                                    securityCode, BPCardOptionalParamSecurityCodeKey,
                                    phoneNumber, BPCardOptionalParamPhoneNumberKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:cardNumber
                                  expirationMonth:8
                                   expirationYear:2025
...
- (void)testValidCardNumbers {
    ...
    for (NSString *number in cardNumbers) {
        BPCard *card = [[BPCard alloc] initWithNumber:number expirationMonth:8 expirationYear:2025];
...
- (void)testValidAmexSecurityCode {
    ...
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    securityCode, BPCardOptionalParamSecurityCodeKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"341111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertNotNil(card, @"Card should not be nil");
    STAssertTrue(card.securityCodeCheck == BPCardSecurityCodeCheckPassed, @"Security code check should pass");
}
...
- (void)testInvalidAmexSecurityCode {
    ...
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    ...
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"341111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertEquals(card.securityCodeCheck, BPCardSecurityCodeCheckFailed, @"Security code check should pass");
}
...
- (void)testSecurityCode {
    ...
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    securityCode, BPCardOptionalParamSecurityCodeKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertEquals(card.securityCodeCheck, BPCardSecurityCodeCheckPassed, @"Security code check should pass");
}
...
- (void)testInvalidSecurityCodeShort {
    ...
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    ...
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertEquals(card.securityCodeCheck, BPCardSecurityCodeCheckFailed, @"Security code check should fail");
}
...
- (void)testSecurityCodeNonNumeric {
    NSString *securityCode = @"junk@!#$@#%!&^*()¨¥ˆ√†∂ß∞´Ω≈®ç";
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    securityCode, BPCardOptionalParamSecurityCodeKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
...
- (void)testSecurityCodeEmptyString {
...
- (void)testSecurityCodeNULL {
    NSString *securityCode = NULL;
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    securityCode, BPCardOptionalParamSecurityCodeKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertEquals(card.securityCodeCheck, BPCardSecurityCodeCheckUnknown, @"Security code check should fail");
}

- (void)testSecurityCodeUnknown {
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    @"Bob Jones", BPCardOptionalParamNameKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertEquals(card.securityCodeCheck, BPCardSecurityCodeCheckUnknown, @"Security code check should be unknown");
}

- (void)testSecurityCodeUnknownWithCardTypeUnknown {
    NSString *securityCode = @"123";
    NSDictionary *optionalFields = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    securityCode, BPCardOptionalParamSecurityCodeKey,
                                    nil];
    BPCard *card = [[BPCard alloc] initWithNumber:@"3911111111111111" expirationMonth:8 expirationYear:2025 optionalFields:optionalFields];
    STAssertEquals(card.type, BPCardTypeUnknown, @"Card type should be unknown");
    STAssertEquals(card.securityCodeCheck, BPCardSecurityCodeCheckUnknown, @"Security code check should be unknown");
}
...
- (void)testNonExpiredCard {
    BPCard *card = [[BPCard alloc] initWithNumber:@"411111111111111112" expirationMonth:8 expirationYear:2025];
...
- (void)testExpiredCardExpiredYear {
    NSDateComponents *components = [[NSCalendar currentCalendar] components:NSYearCalendarUnit fromDate:[NSDate date]];
    BPCard *card = [[BPCard alloc] initWithNumber:@"411111111111111112" expirationMonth:8 expirationYear:components.year-1];
    STAssertTrue(card.expired, @"Card should be expired");
}
...
- (void)testValidWithEverythingValid {
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:2025];
...
- (void)testValidWithVaidCardNumberAndExpired {
    NSDateComponents *components = [[NSCalendar currentCalendar] components:NSYearCalendarUnit fromDate:[NSDate date]];
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111" expirationMonth:8 expirationYear:components.year-1];
    STAssertFalse(card.valid, @"Card should not be valid");
...
- (void)testValidWithNothingValid {
    BPCard *card = [[BPCard alloc] initWithNumber:@"4111111111111111111" expirationMonth:13 expirationYear:1900];
    STAssertFalse(card.valid, @"Card should not be valid");
    STAssertTrue([card.errors count] > 0, @"Card error count should be > 0");
}
